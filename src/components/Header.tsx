...
import { IconMenu, LogoMark, IconCalendar, IconSearch, IconShield, IconChurch } from './icons'
...
export default function Header() {
  ...
  return (
    <div className="bg-white/95 backdrop-blur border-b sticky top-0 z-40 shadow-sm">
      <div className="px-4 py-2 grid grid-cols-3 items-center">
        <div className="flex items-center gap-2">
          <span className="inline-flex items-center justify-center w-8 h-8 rounded-xl bg-blue-100 text-blue-600"><LogoMark className="w-5 h-5" /></span>
          <div className="text-base md:text-lg font-semibold">Church Kids Check-In</div>
        </div>
        ...
        <div className="flex justify-end">
          <button aria-label="Menu" className="p-2 rounded-md bg-gray-100 hover:bg-gray-200" onClick={openMenu}>
            <IconMenu />
          </button>
        </div>
      </div>
      {open && createPortal((
        <div className="fixed inset-0 z-[100]">
          ...
          <div className={(slide? 'translate-x-0' : 'translate-x-full') + " absolute right-0 top-0 h-full w-[360px] sm:w-[420px] bg-gray-50 dark:bg-gray-900 shadow-2xl border-l border-gray-200 dark:border-gray-800 transition-transform duration-200"}>
            <div className="h-full flex flex-col">
              <div className="sticky top-0 z-10 bg-white/95 dark:bg-gray-900/95 backdrop-blur px-4 py-3 border-b border-gray-200 dark:border-gray-800 flex items-center justify-between">
                ...
                <button className="px-2 py-1 rounded bg-gray-100 hover:bg-gray-200 dark:bg-gray-800 dark:hover:bg-gray-700" onClick={closeMenu}>✕</button>
              </div>
              <div className="p-4 overflow-y-auto">
                <div className="flex items-center justify-between px-3 py-2 rounded-xl border shadow-sm bg-white">
                ...
                <div className="mt-2 grid gap-2">
                  <button className="flex items-center justify-between px-3 py-2 rounded-xl border shadow-sm bg-white hover:bg-gray-50 border-gray-200 dark:bg-gray-800 dark:border-gray-700 dark:hover:bg-gray-700" onClick={async()=>{
                  const rows = await listAll().catch(()=>[])
                  if (!rows.length) return exportLocalCsv()
                  ...
                  closeMenu()
                    ...
                    <span className="text-xs px-2 py-1 rounded bg-emerald-600 text-white">Action</span>
                  </button>
